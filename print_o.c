...
/*   Updated: 2019/07/19 13:37:16 by ibotnaru         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char        *return_buffer_o1(va_list ap, t_flags *all_flags)
{
    ...
    str_num = ft_itoa_base((int)num, 8, 0);
    if (all_flags->hash_flag == 1 && all_flags->minus_flag == 1)
        buffer = flag_hash_and_minus_on_o(all_flags, str_num);
    else if (all_flags->hash_flag == 1 && all_flags->zero_flag == 1)
        buffer = flag_zero_and_hash_on_o(all_flags, str_num);
    else if (all_flags->zero_flag == 1)
        buffer = flag_zero_on_o(all_flags, str_num);
    else if (all_flags->hash_flag == 1)
        buffer = flag_hash_on1_o(all_flags, str_num);
    else if(all_flags->minus_flag == 1)
        buffer = flag_minus_on_o(all_flags, str_num);
    else
        buffer = return_buffer_o2(all_flags, str_num);
    return (buffer);
}